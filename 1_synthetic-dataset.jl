...
savepath = "./figures/synthetic"
if !ispath(savepath)
    mkpath(savepath)
end


# potential avenue: randomly sample points in the n-simplex.
# This could be done with a Dirichlet distribution or some
# To guarantee "uniformity", one could use a dirichlet with αᵢ=1 ∀i

# We would have to adjust how we generate the RBF points though to
# guarantee nothing "too close" to the vertices- perhaps rejection
# sampling?

# Additionally, we can comment on

# could we use sparse matrices to speed things up?
...
for i ∈ axes(X, 1)
    ...
    Xnl[i,:] .= (abund[1,i] .* Rr) .+ (abund[2,i] .* Rg) .+ (abund[3,i] .* Rb) .+ (abund[1,i] * abund[2,i] .* Rr .* Rg)  .+ (abund[1,i] * abund[2,i] .* Rr .* Rg) .+ (abund[1,i] * abund[3,i] .* Rr .* Rb)  .+ (abund[2,i] * abund[3,i] .* Rg .* Rb)
end
